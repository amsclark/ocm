...
/*	cms/css.php is a dead endpoint. In the configuration this repository
	ships it has answered HTTP 500 to every request since the initial
	import.
	...
	pika_init() prepends two RELATIVE entries to the include_path,
	'./app/lib' and './app/extralib', so the library directory only
	resolves while the working directory is cms/. After the chdir the
	working directory is the document root, which has no app/lib, and the
	next thing pika_init() does is require pl.php:

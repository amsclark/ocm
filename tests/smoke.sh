...
if [ "$HAVE_DB" = 1 ]; then
	...
	if ! command -v openssl >/dev/null 2>&1; then
		...
		# Whichever character the signature ends in, this is not it. Writing a
		# literal 0 here matched the real signature one run in sixteen, and
		# that run reported the gate as broken.
		case "$good_sig" in
...
# ── 31. The poverty guideline and the two password policy settings ─────────
